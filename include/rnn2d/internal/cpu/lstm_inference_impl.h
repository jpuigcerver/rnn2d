...
#ifndef RNN2D_INTERNAL_CPU_LSTM_INFERENCE_IMPL_H_
#define RNN2D_INTERNAL_CPU_LSTM_INFERENCE_IMPL_H_
...
class LstmInferenceCpuImpl {
 public:
  ...
  rnn2dStatus_t Forward() {
    ...
    for (int z = 0; z < 4; ++z) {
      ...
      gemm<T>(
          'N', 'N', H_ * W_ * N_, 5 * D_, K_,
          1.0, input_, K_, W(P, z, 0, 0, 0), 5 * D_,
          1.0, Q(z, 0, 0, 0, 0, 0), 5 * D_);
    }
 ...
 protected:
  ...
  T* Q(const int z, const int y, const int x, const int n, const int g,
       const int d) {
    return reinterpret_cast<T*>(wspace_) +
        z * H_ * W_ * N_ * 5 * D_ +
        y * W_ * N_ * 5 * D_ +
        x * N_ * 5 * D_ +
        n * 5 * D_ +
  ...
  const T* Q(const int z, const int y, const int x, const int n, const int g,
             const int d) const {
    return reinterpret_cast<const T*>(wspace_) +
        z * H_ * W_ * N_ * 5 * D_ +
        y * W_ * N_ * 5 * D_ +
        x * N_ * 5 * D_ +
        n * 5 * D_ +
...
#endif // RNN2D_INTERNAL_CPU_LSTM_INFERENCE_IMPL_H_
