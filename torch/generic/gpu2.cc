...
#define TH_GENERIC_FILE "generic/gpu.cc"
#else
...
TORCH_API int THCTensor_(rnn2d_lstm_bw_input)(lua_State* L){
  ...
  const int D = THCTensor_(nElement)(d_workspace) / (H * W * N * 4 * 6);
  CHECK_TENSOR_N_ELEMENTS("gradWorkspace", 4 * H * W * N * D * 6,
                          THCTensor_(nElement)(d_workspace));
  // Check parameters tensor
  CHECK_TENSOR_N_ELEMENTS("parameters", 4 * (1 + K + D + D) * 5 * D,
                          THCTensor_(nElement)(param));
  // Check that all tensors are contiguous
  CHECK_TENSOR_CONTIGUOUS("gradInput", THCTensor_(isContiguous)(d_inp));
  CHECK_TENSOR_CONTIGUOUS("parameters", THCTensor_(isContiguous)(param));
  CHECK_TENSOR_CONTIGUOUS("gradWorkspace",
                          THCTensor_(isContiguous)(d_workspace));
  // Run backward pass
  rnn2d_lstm_bw_input_cpu<real>(
      H, W, N, K, D,
      THCTensor_(data)(param),
      THCTensor_(data)(d_workspace),
      scale, THCTensor_(data)(d_inp));
  return 0;
}
...
TORCH_API int THCTensor_(rnn2d_lstm_bw_params)(lua_State* L){
  ...
  CHECK_TENSOR_N_ELEMENTS("gradParameters", 4 * (1 + K + D + D) * 5 * D,
                          THCTensor_(nElement)(d_param));
  // Check that all tensors are contiguous
  CHECK_TENSOR_CONTIGUOUS("input", THCTensor_(isContiguous)(inp));
  CHECK_TENSOR_CONTIGUOUS("output", THCTensor_(isContiguous)(out));
  CHECK_TENSOR_CONTIGUOUS("gradParameters", THCTensor_(isContiguous)(d_param));
  CHECK_TENSOR_CONTIGUOUS("gradWorkspace",
                          THCTensor_(isContiguous)(d_workspace));
  // Run backward pass
